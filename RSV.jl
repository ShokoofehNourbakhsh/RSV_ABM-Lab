...
mutable struct Human
    ...
    age::Int64          # in yeras
    age_group::Int64    # G1 = < 1, G2= 2, G3= 3, G4= 4, G5= 5:19, G6= > 19
...
function apply_charectristics(x::Human)
    x.age_group = rand(agedist)
    x.age = rand(agebraks[x.group])

    if x.age_group == 1   #assign preterm if < 1 years old
        x.preterm = rand(predist)
    end

    if x.age_group == 7  #assign smoker if 19+ years old
